...
# extract BED files:
samtools view -q 10 -b esc_med1.bam  | bedtools bamtobed | grep -v 'chrM' |awk '{FS=OFS="\t"; print $1,$2,$3,".",0,$6}' - |awk '!x[$0]++' -| gzip >esc_med1.bed.gz
samtools view -q 10 -b esc_med12.bam | bedtools bamtobed | grep -v 'chrM' |awk '{FS=OFS="\t"; print $1,$2,$3,".",0,$6}' - |awk '!x[$0]++' -| gzip >esc_med1.bed.gz

# Make the FLAT files:
python3 -c "import glbase3, sys; glbase3.bed_to_flat(sys.argv[1].split(', '), '%s.flat' % sys.argv[2], name=sys.argv[2], isPE=False, read_extend=200, strand=True, gzip=True)"  esc_med1.bed.gz esc_med1.flat
python3 -c "import glbase3, sys; glbase3.bed_to_flat(sys.argv[1].split(', '), '%s.flat' % sys.argv[2], name=sys.argv[2], isPE=False, read_extend=200, strand=True, gzip=True)"  esc_med12.bed.gz esc_med12.flat
